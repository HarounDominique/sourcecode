...
V3: produces execution_paths (ordered sequences), not just component edges.
One path per entry point; cut when evidence ends. No inference, no naming.
"""
...
def _has_code_evidence(clean: str, class_name: str) -> bool:
    ...
    if re.search(rf"\b{esc}\s*\(", clean):
        return True
    # Import + at least one usage on a non-import, non-comment-remnant line
...
def analyze_execution_paths(
    ...
    """Build ordered execution paths from PR-changed files.

    Rules:
    - One path per entry point (most evident service, not all)
    - Each step requires direct code evidence
    - Path ends when evidence runs out (no inference gap-filling)
    - Forward-only: Controller → Service → Repository

    Returns list of: {name, entry_point, path, end_state}
    ...
    for entry_path in entry_files[:max_paths]:
        ...
        # Pick the MOST EVIDENT (earliest-referenced) service — single path only
        evidenced_svcs = _find_evidenced_ordered(root, entry_path, all_services)
        if not evidenced_svcs:
            continue  # No verifiable downstream → no path

        path_steps: list[str] = []
        seen: set[str] = set()

        svc_class, svc_method = evidenced_svcs[0]  # one service, most evident
        svc_label = _step_label(svc_class, svc_method)
        path_steps.append(svc_label)
        seen.add(svc_label)

        # Resolve service file to look for repository evidence
        ...
        if svc_path:
            ...
            if evidenced_repos:
                ...
                if repo_label not in seen:
                    path_steps.append(repo_label)

        paths.append({
            "name": _path_name(entry_class),
            "entry_point": entry_point_str,
            "path": path_steps,
            "end_state": _detect_end_state(path_steps),
        })

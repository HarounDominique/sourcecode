...
def _emit_error_json(
    ...
    **context: object,
    ...
    import sys as _sys
    payload = build_error_envelope(error, message, hint=hint, expected=expected, **context)
...
try:
    ...
    def _json_click_usage_error_show(self: Any, file: Any = None) -> None:  # type: ignore[override]
        ...
        import sys as _jse
        _code_map = {
            "NoSuchOption": INVALID_INPUT_CODE,
            "BadOptionUsage": INVALID_INPUT_CODE,
            "BadParameter": INVALID_INPUT_CODE,
            "MissingParameter": INVALID_INPUT_CODE,
            "BadArgumentUsage": INVALID_INPUT_CODE,
        }
        code = _code_map.get(type(self).__name__, INVALID_INPUT_CODE)
...
def main(
    ...
    if mode in _DEPRECATED_MODES:
        ...
        _emit_error_json(
            ...
            hint="Pass one of the supported --mode values.",
    ...
    if rank_by not in _RANK_CHOICES:
        _emit_error_json(
            ...
            hint="Pass one of the supported --rank-by values.",
    ...
    if symbol is not None and not symbol.strip():
        _emit_error_json(
            ...
            hint="Provide a symbol name or omit --symbol.",
    ...
    if symbol and mode not in ("contract", "standard"):
        _emit_error_json(
            INVALID_INPUT_CODE,
            f"--symbol requires --mode contract or standard (got '{mode}'). Symbol search uses the contract pipeline which does not run in raw mode.",
            hint="Switch --mode to contract or standard before using --symbol.",
            expected="--mode contract or --mode standard.",
            flag="--mode",
            value=mode,
        )
        raise typer.Exit(code=2)  # FIX-P2-7: arg validation → exit 2
    ...
    if entrypoints_only and mode not in ("contract", "standard"):
        _emit_error_json(
            ...
            hint="Switch --mode to contract or standard before using --entrypoints-only.",
            expected="--mode contract or --mode standard.",
            flag="--mode",
            value=mode,
        )
    ...
    if compact and full:
        _emit_error_json(
            INVALID_INPUT_CODE,
            "--compact and --full are mutually exclusive. --compact produces a bounded summary; --full removes truncation limits and is meant for --agent mode. Use --agent --full for expanded output.",
    ...
    if format not in FORMAT_CHOICES:
        _emit_error_json(
            ...
            hint="Choose a supported --format value.",
    ...
    if graph_detail not in GRAPH_DETAIL_CHOICES:
        _emit_error_json(
            ...
            hint="Choose a supported --graph-detail value.",
    ...
    if docs_depth not in DOCS_DEPTH_CHOICES:
        _emit_error_json(
            ...
            hint="Choose a supported --docs-depth value.",
    ...
    if parsed_graph_edges is not None:
        ...
        if invalid_edges:
            _emit_error_json(
                INVALID_INPUT_CODE,
                f"Invalid values for --graph-edges: {', '.join(invalid_edges)}. Valid options: {', '.join(sorted(GRAPH_EDGE_CHOICES))}.",
                hint="Choose only supported --graph-edges values.",
                expected=f"Subset of: {', '.join(sorted(GRAPH_EDGE_CHOICES))}",
                flag="--graph-edges",
                value=invalid_edges,
...
def prepare_context_cmd(
    ...
    if format is not None and format not in _PC_FORMAT_CHOICES:
        _emit_error_json(
            INVALID_INPUT_CODE,
            f"Invalid value '{format}' for --format. Valid options: {', '.join(_PC_FORMAT_CHOICES)}.",
            hint="Choose a supported prepare-context output format.",
    ...
    if not target.exists() or not target.is_dir():
        _emit_error_json(
            ...
            hint="Pass a repository directory that exists on disk.",
            expected="An existing directory path.",
            path=str(target),
        )
        raise typer.Exit(code=1)
...
def mcp_init(
    ...
    if target:
        ...
        if not clients:
            ...
            _emit_error_json(
                INVALID_INPUT_CODE,
                f"Unknown target '{target}'. Valid: {valid}.",
                hint="Choose one of the detected client targets.",
                expected=f"One of: {valid}" if valid else "A detected MCP client target.",
                flag="--target",
                value=target,
                valid_values=[c.slug for c in all_clients],
            )
            raise typer.Exit(code=1)
    else:
        clients = all_clients

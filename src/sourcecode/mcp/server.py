...
All tools return:
  {"success": bool, "data": dict | str | None, "error": {"code": str, "message": str, "hint": str, "expected": str} | None}
...
from sourcecode.mcp.runner import CommandError
...
MCP_COMPATIBILITY_SCHEMA_VERSION = "1"
...
def start_session(repo_path: str = ".", task_description: str = "") -> dict:
    ...
    try:
        if not isinstance(repo_path, str):
            return _err(
                "repo_path must be a string.",
                INVALID_INPUT_CODE,
                hint="Pass repo_path as a string path.",
                expected="A string repository path.",
            )
    ...
    try:
        ...
        if not isinstance(task_description, str) or not task_description.strip():
            return _err(
                ...
                INVALID_INPUT_CODE,
                hint="Provide a task description with enough detail to infer intent.",
...
def get_compact_context(repo_path: str = ".", git_context: bool = False) -> dict:
    ...
    try:
        ...
        if not isinstance(git_context, bool):
            return _err(
                "git_context must be boolean.",
                INVALID_INPUT_CODE,
                hint="Pass git_context as true or false.",
    ...
    try:
        ...
        if not isinstance(module, str) or not module.strip():
            return _err(
                ...
                INVALID_INPUT_CODE,
                hint="Pass a subdirectory name relative to the repository root.",
                expected="A non-empty module path.",
            )
        repo_path = _normalize_repo_path(repo_path)
        _path_err = _check_repo_path(repo_path)
        if _path_err is not None:
...
def generate_tests_context(repo_path: str = ".", include_all: bool = False) -> dict:
    ...
    try:
        ...
        if not isinstance(include_all, bool):
            return _err(
                "include_all must be boolean.",
                INVALID_INPUT_CODE,
                hint="Pass include_all as true or false.",
                expected="A boolean value.",
            )
        repo_path = _normalize_repo_path(repo_path)
        _path_err = _check_repo_path(repo_path)
...
def get_impact_context(repo_path: str = ".", target: str = "", depth: int = 4) -> dict:
    ...
    try:
        ...
        if not isinstance(target, str) or not target.strip():
            return _err(
                "target must be a non-empty class name or FQN.",
                INVALID_INPUT_CODE,
                hint="Pass a Java class name or file path to analyze.",
                expected="A non-empty target value.",
            )
        if not isinstance(depth, int) or depth < 1 or depth > 8:
            return _err(
                "depth must be an integer between 1 and 8.",
                INVALID_INPUT_CODE,
                hint="Use a BFS depth between 1 and 8.",
...
def modernize_context(repo_path: str = ".", format: str = "json") -> dict:
    ...
    try:
        ...
        if not isinstance(format, str) or format != "json":
            return _err(
                "format must be 'json' — yaml is not supported for modernize output.",
                INVALID_INPUT_CODE,
                hint="Use the supported JSON output format.",
                expected="The literal string 'json'.",
                flag="format",
                value=format,
            )
        repo_path = _normalize_repo_path(repo_path)
...
def version() -> dict:
    ...
    """
    result = _execute(["version"])
    if isinstance(result, CallToolResult):
        return result
    cli_output = result.get("data")
    cli_version = ""
    if isinstance(cli_output, str):
        cli_version = cli_output.strip().split()[-1] if cli_output.strip() else ""
    return _ok({
        "cli_version": cli_version or _sourcecode_version,
        "cli_output": cli_output,
        "mcp_server_version": _sourcecode_version,
        "compatibility_schema_version": MCP_COMPATIBILITY_SCHEMA_VERSION,
    })
...
def telemetry(action: str) -> dict:
    ...
    if action not in _TELEMETRY_ACTIONS:
        return _err(
            ...
            INVALID_INPUT_CODE,
            hint="Choose one of the supported telemetry actions.",
            expected=f"One of: {', '.join(sorted(_TELEMETRY_ACTIONS))}",
            flag="action",
            value=action,
        )
...
def _finalize_mcp_registry() -> None:
    ...
    # Remove the manually-decorated tools so the runtime registry becomes the
    # single source of truth exposed by MCP.
    try:

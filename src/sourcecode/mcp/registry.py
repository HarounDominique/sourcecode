...
def _generic_argv_builder(path: tuple[str, ...], params: tuple[ToolParamSpec, ...]) -> Callable[[Mapping[str, Any]], list[str]]:
    def _builder(inputs: Mapping[str, Any]) -> list[str]:
        ...
        for param in params:
            ...
            # Boolean flags are emitted only when truthy.
            if param.is_flag:
                if bool(value):
                    argv.append(param.option_names[0])
                continue
...
def _canonical_spec_for_runtime_command(runtime: RuntimeCommand) -> ToolSpec:
    ...
    return ToolSpec(
        ...
        docstring=_build_contract_doc(description, params, runtime.path, tuple(dict.fromkeys(supported_targets)), tuple(dict.fromkeys(unsupported_targets))),
...
def _prepare_context_aliases() -> list[ToolSpec]:
    ...
    return [
        ...
        _alias_spec(
            ...
            (
                ToolParamSpec("repo_path", "argument", str, required=False, default=".", is_path=True),
                ToolParamSpec("format", "option", str, required=False, default="json", option_names=("--format",)),
            ),
...
def build_tool_specs() -> tuple[ToolSpec, ...]:
    ...
    # Deduplicate by name, preferring canonical runtime-derived tools.
...
def validate_registry() -> list[str]:
    ...
    if not runtime_paths <= registry_paths:
        missing = sorted(" ".join(path) or "sourcecode" for path in runtime_paths - registry_paths)
        issues.append(f"missing_mcp_tools_for_cli_commands: {missing}")

    for spec in build_public_tool_specs():
        if spec.name != _tool_name_for_path(spec.cli_path):
            continue
        runtime = next((r for r in discover_runtime_commands() if r.path == spec.cli_path), None)
        if runtime is None:
            issues.append(f"tool_without_runtime_command: {spec.name}")
            continue
        if runtime.callback is None and runtime.path != ():
            continue

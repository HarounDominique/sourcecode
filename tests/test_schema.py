...
def test_compact_has_no_file_tree():
    """v0.24.0: compact_view no longer includes file_tree_depth1 — noise reduction."""
    sm = SourceMap(
        file_tree={
...
def test_c8_compact_has_no_file_tree_depth1():
    """C8 (v0.24.0): compact_view no longer includes file_tree_depth1 — removed for noise reduction."""
    sm = SourceMap()
    result = compact_view(sm)
    assert "file_tree_depth1" not in result
